...
try {
    ...
    // Ambil data pengguna dengan casting UUID
    ...
    // Ambil artikel milik user yang sedang login dengan casting UUID
    $query = "SELECT * FROM articles WHERE user_id = :user_id::uuid ORDER BY created_at DESC";
    ...
    $no_articles_message = empty($articles) ? "Anda belum memiliki artikel." : "";
...
// Pesan sukses/error dari operasi sebelumnya
            ...
            <div class="p-4">
                ...
                <nav>
                    <ul class="space-y-4">
                        ...
                        <li>
                            <a class="text-white hover:text-blue-400 <?php echo (basename($_SERVER['PHP_SELF']) == 'edit_profile.php') ? 'text-blue-500' : ''; ?>"
            ...
            <main class="flex-grow p-8">
                ...
                <section class="bg-gray-800 p-6 rounded-lg shadow-lg mb-8">
                    <div class="flex items-center space-x-6">
                        <img alt="Profile picture" class="w-32 h-32 rounded-full"
                            src="<?php echo getAvatar($user['avatar'], $user['username']); ?>"
                        ...
                        <div>
                            ...
                            <p class="text-gray-400">
                                <?php echo isset($user['job']) ? htmlspecialchars($user['job']) : 'No Job Specified'; ?>
                            </p>
                            <div class="mt-4 flex space-x-4">
                                <?php if (!empty($user['instagram'])): ?>
                                    <a class="text-blue-400 hover:text-blue-600"
                                        href="<?php echo htmlspecialchars($user['instagram']); ?>" target="_blank">
                                        <i class="fab fa-instagram"></i>
                                    </a>
                                <?php endif; ?>
                                <?php if (!empty($user['linkedin'])): ?>
                                    <a class="text-blue-400 hover:text-blue-600"
                                        href="<?php echo htmlspecialchars($user['linkedin']); ?>" target="_blank">
                                        <i class="fab fa-linkedin"></i>
                                    </a>
                                <?php endif; ?>
                                <?php if (!empty($user['tiktok'])): ?>
                                    <a class="text-blue-400 hover:text-blue-600"
                                        href="<?php echo htmlspecialchars($user['tiktok']); ?>" target="_blank">
                                        <i class="fab fa-tiktok"></i>
                                    </a>
                                <?php endif; ?>
                                <?php if (!empty($user['twitter'])): ?>
                                    <a class="text-blue-400 hover:text-blue-600"
                                        href="<?php echo htmlspecialchars($user['twitter']); ?>" target="_blank">
                    ...
                    <div class="mt-6">
                        ...
                        <div class="flex items-center">
                            ...
                            <button id="editBioBtn" class="ml-4 text-blue-400 hover:text-blue-600">
                                <i class="fas fa-pencil-alt"></i>
                            </button>
                        </div>
                        <div id="bioEdit" class="hidden mt-2">
                            <textarea id="bioInput" class="w-full p-2 rounded bg-gray-700 text-white"
                                rows="4"><?php echo htmlspecialchars($user['biography']); ?></textarea>
                ...
                <div class="flex justify-between items-center mb-6">
                    ...
                    <a class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded"
                        href="create_article.php">Create Article</a>
                </div>
                    ...
                    <?php foreach ($articles as $article): ?>
                        <div class="card">
                            ...
                            <p class="text-gray-400 mb-4">
                                ...
                                // Add ellipsis if description is longer than 20 words
                                if (count($words) > 20) {
                                    echo '...';
                                }
                                ?>
                            ...
                            <div class="flex justify-between items-center">
                                <a class="text-blue-400 hover:underline"
                                    href="edit_article.php?id=<?php echo $article['id']; ?>">Edit</a>
                                <a class="text-red-400 hover:underline"
                                    href="delete_article.php?id=<?php echo $article['id']; ?>">Delete</a>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
...
<script>
    // JavaScript untuk mengedit biografi
        ...
        fetch('save_bio.php', {
            ...
            .then(data => {
                if (data.message) {
                    ...
                    // Hapus alert dan hanya lakukan pembaruan tampilan
                }
            })
            .catch(error => {
